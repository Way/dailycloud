        ...
        if(this.hasFocus && (!this.gecko || force)) {
            ...
            if(e.keyCode == 40) {
                ...
                if(e.shiftKey) {
                    ...
                    log('lineDown', this.selection, newBound, this.cursor.getPosition());
                }
                return;
            }
            if(e.keyCode == 38) {
                ...
                if(e.shiftKey) {
                    ...
                    log('lineUp', this.selection, newBound, this.cursor.getPosition());
                }
                return;
            }
            if(e.keyCode == 37) {
...
Textile.History = Textile.Utils.makeClass({
    ...
    redo : function() {
        var lastCommand = this.commands[++this.date];
        if(this.date > this.commands.length - 1)
            this.date = this.commands.length - 1;
        if(!lastCommand)
            return;
        // no more history
