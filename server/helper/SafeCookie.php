 ...
 * @class     -   SafeCookie
 * @file      -   SafeCookie.php
 * @location  -   ./server/helper/SafeCookie
 * @package   -   dailyd
 ...
 */
class SafeCookie implements SafeCookieInterface {
    private $pass_key;
    ...
    public function setCookie($cookie_name, $cookie_value, $seconds_alive = 1800, $path = '/') {
        ...
        $ancrypt = new ANCrypt($this->pass_key);

        $encrypted_value = $ancrypt->encrypt($cookie_value_valid);

        // Set Cookie
        if (self::$cookie_params == null) {
            self::$cookie_params = session_get_cookie_params();
        }
        setcookie($name_hash, $encrypted_value, time() + $seconds_alive, $path, self::$cookie_params['domain'], self::$cookie_params['secure'], self::$cookie_params['httponly']);
    }
    ...
    public function validateCookie($cookie_name) {
        ...
        // Get Cookie Value
        $cookie_value = isset($_COOKIE[$name_hash]) ? $_COOKIE[$name_hash] : null;
    ...
    public static function set($cookie_details, $pass_key = null) {
        ...
        if (is_array($cookie_details) && count($cookie_details) >= 2 && !empty($pass_key)) {
            $safe_cookie = new SafeCookie($pass_key);
